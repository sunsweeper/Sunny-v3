...
<main className="relative flex min-h-screen flex-col overflow-hidden bg-slate-950 text-white">
  ...
  <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(250,204,21,0.25),transparent_55%),linear-gradient(135deg,#0f172a_0%,#020617_45%,#1e293b_100%)]" />
  ...
  {/* The rest of your chat UI goes here – messages, input, etc. */}
  {/* ... */}
</main>
            Online
          </span>
...
<section className="relative z-10 mx-auto flex w-full max-w-5xl flex-1 flex-col gap-6 px-4 py-6">
  <div className="flex-1 space-y-5 overflow-y-auto rounded-3xl border border-white/10 bg-black/70 p-6 shadow-[0_20px_80px_rgba(0,0,0,0.35)] backdrop-blur">
    ...
    {/* Your message list / chat bubbles go here */}
    {/* ... rest of your chat content ... */}

  </div>

  {/* Input area would come after this div */}
</section>
            </p>
          )}
...
{isLoading && (
  <div className="flex items-start gap-3">
    <div className="relative h-9 w-9 overflow-hidden rounded-full border border-white/20 shrink-0">
      <Image
        src="/sunny-avatar.png"
        alt="Sunny the Silkie chicken"
        fill
...
<div className="rounded-3xl border border-white/10 bg-black/70 p-4 shadow-[0_12px_40px_rgba(0,0,0,0.3)] backdrop-blur">
  {/* Your content goes here – e.g. input field, send button, or other bottom elements */}
  {/* ... */}
</div>
...
<input
  ...
  className="min-h-[52px] flex-1 resize-none rounded-full border border-slate-700 bg-slate-900/80 px-5 py-3 text-sm text-white placeholder:text-white/40 focus:border-amber-300/70 focus:outline-none focus:ring-2 focus:ring-amber-300/30 disabled:opacity-50"
  disabled={isLoading}
/>
            />
            <button
